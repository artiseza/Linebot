...
Qdict={'q1':['目前有沒有高血壓','沒有','有，沒有治療','有，不規則治療','有，規則治療'], #qtag = 0
        'q2':['目前有沒有糖尿病','沒有','有，沒有治療','有，不規則治療','有，規則治療'],
        'q3':['目前有沒有高血脂','沒有','有，沒有治療','有，不規則治療','有，規則治療'],
        'q4':['目前是否有治療中的內外科疾病','沒有','有，請輸入:'],
        'q5':['曾看過精神科（身心科），被確診過任何精神心理疾病？','沒有','焦慮症(廣泛性焦慮症、恐慌症、懼曠症)','憂鬱症','躁鬱症','飲食疾患(如:暴食症、厭食症)','注意力缺乏/過動症','思覺失調症(精神分裂症)','強迫症','創傷後壓力症候群','其它'],
        ...
        '1':['1:這兩週，你通常幾點上床準備睡覺？\n請輸入: 24時制','(例如: 2400)'], #qtag = 14
         '2':['2:這兩週，你上床經過多久才睡著?\n請輸入: 分鐘','(例如: 30)'],
         '3':['3:這兩週，你醒來後通常幾點下床?\n請輸入: 24時制','(例如: 0730)'],
         '4':['4:這兩週，你實際每晚平均可以入睡幾小時？\n請輸入: 小時/分鐘數','(例如: 8/30)'],
         '5':['5:整體而言，您滿意這兩週的睡眠品質？','很滿意','滿意','還可以','不滿意','很不滿意'],
         '6A':['6A:睡眠是否出現下列困擾','無法在三十分鐘內入睡','無法維持睡眠，半夜會醒來','比你預計的時間早醒來','沒有以上困擾'], #qtag = 19
            '6-1-a':['6-1-a:無法在三十分鐘內入睡','每週不到一次','每週一兩次','每週三次以上'],
            '6-1-b':['6-1-b:關於無法在三十分鐘內入睡的嚴重程度，你的評估？','還算輕微','有些困擾','明顯影響','非常嚴重'],
            '6-2.1-a':['6-2.1-a:無法維持睡眠，半夜會醒來','每週不到一次','每週一兩次','每週三次以上'],
            '6-2.1-b':['6-2.1-b:關於無法維持睡眠的嚴重程度，你的評估？','還算輕微','有些困擾','明顯影響','非常嚴重'],
            '6-2.2-a':['6-2.2-a:比你預計的時間早醒來(太早醒)','每週不到一次','每週一兩次','每週三次以上'],
            '6-2.2-b':['6-2.2-b:關於太早醒的嚴重程度，你的評估？','還算輕微','有些困擾','明顯影響','非常嚴重'],
        '6B':['6B:接下來請你回想這兩週的睡眠期間是否受以下症狀干擾？','半夜要起來上廁所','覺得呼吸不順暢','大聲打鼾','咳嗽','發冷','燥熱','作惡夢','身上有疼痛','沒有以上困擾'], #qtag = 20
            '6-3':['6-3:這兩週中，半夜要起來上廁所的狀況','每週不到一次','每週一兩次','每週三次以上'],
            ...
            '6-4':['6-4:這兩週中，呼吸不順暢對睡眠的干擾','每週不到一次','每週一兩次','每週三次以上'],
            '6-5.1':['6-5.1:這兩週中，大聲打鼾對睡眠的干擾','每週不到一次','每週一兩次','每週三次以上'],
            '6-5.2':['6-5.2:這兩週中，咳嗽對睡眠的干擾','每週不到一次','每週一兩次','每週三次以上'],
            ...
            '6-6':['6-6:這兩週中，覺得冷對睡眠的干擾','每週不到一次','每週一兩次','每週三次以上'],
            '6-7':['6-7:這兩週中，覺得躁熱對睡眠的干擾','每週不到一次','每週一兩次','每週三次以上'],
            '6-8':['6-8這兩週中，作惡夢對睡眠的干擾:','每週不到一次','每週一兩次','每週三次以上'],
            '6-9':['6-9:這兩週中，身上有疼痛對睡眠的干擾','每週不到一次','每週一兩次','每週三次以上'],        
        ...
        '11_2':['11_2:過去兩週來，是否需要使用酒精或其他中藥或保健食品幫忙睡眠？','從未發生','每週不到一次','每週一兩次','每週三次以上'], #qtag = 26
        ...
        '29':['29:這兩週，你是否會有不如死掉或用某種方式傷害自己的念頭','從未','很少','一半以上','幾乎都會'], #qtag = 51
        'gender':['基本資料\n性別 :','男','女'], #qtag = 52
        'birthday':['生日 :\n請輸入: 民國/月/日:','(例如: 60/1/1)'],
        'height':['身高 :\n請輸入: 公分 cm','(例如: 170)'],
        'weight':['體重 :\n請輸入: 公斤 kg','(例如: 50)'],
        ...
        'income':['您家庭最近一年每月平均總收入為多少元？','30,000元以下','30,001~60,000元','60,001~100,000元','100,000元以上'] #qtag = 60
        }
